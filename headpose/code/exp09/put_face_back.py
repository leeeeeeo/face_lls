...
def drawPointsOnImg(points, img, color, cover=True):
    if cover == False:
    ...
    if type(points) == types.ListType:
        ...
        if color == 'r':
            cv2.circle(imgTmp, points, 1, (0, 0, 255), -1)
        elif color == 'g':
            cv2.circle(imgTmp, points, 1, (0, 255, 0), -1)
        else:
            cv2.circle(imgTmp, points, 1, (255, 0, 0), -1)
    elif type(points) == type(np.array([])):
        if points.shape[0] == 1 or points.shape[1] == 1:
            if color == 'r':
                cv2.circle(imgTmp, (int(points[0]), int(
                    points[1])), 1, (0, 0, 255), -1)
            elif color == 'g':
                cv2.circle(imgTmp, (int(points[0]), int(
                    points[1])), 1, (0, 255, 0), -1)
            else:
                cv2.circle(imgTmp, (int(points[0]), int(
                    points[1])), 1, (255, 0, 0), -1)
        else:
            for point in points:
                if color == 'r':
                    cv2.circle(imgTmp, (int(point[0]), int(
