...
def rotateYZPlane(ptY, ptZ, center, angle,imgWidth=640,imgHeight=640):
    ptY = ptY
    ptZ = imgHeight-ptZ
    centerY = center[0]
...
def nodHR(objLines,nodAngle,nodCenterMode):
    nodCenter = findNodCenter(objLines, nodCenterMode)
    nodCenter=(nodCenter[0]+400,nodCenter[1])
    nodedObjLines = nod3DModel(objLines, nodCenter, nodAngle)
    return nodedObjLines



def nodHair(objPath, nodAngle, nodCenterMode, hairVLines, hairVLinesTxtPath):
    '''read origin obj file'''
    objLines, _, _ = readObj(objPath)
    '''1. find nod center'''
    nodCenter = findNodCenter(objLines, nodCenterMode)
    '''2. nod every point in YZ plane around nodCenter'''
    nodedObjLines = nod3DModel(hairVLines, nodCenter, nodAngle)
    '''3. save noded txt file'''
    nodTxtPath = '{}-nod.txt'.format(os.path.splitext(hairVLinesTxtPath)[0])
    saveTxtFile(nodTxtPath, nodedObjLines)
    return nodedObjLines


def nodHairHR(objLines, nodAngle, nodCenterMode, hairPoints, minZ=60.0):
    nodHairPoints = []
    nodCenter = findNodCenter(objLines, nodCenterMode)
    for (x, y) in hairPoints:
        ptY = float(y)
        ptZ = minZ
        nodY, nodZ = rotateYZPlane(ptY, ptZ, nodCenter, nodAngle)
        nodHairPoints.append((x, nodY))
    return nodHairPoints

def nodHairHR_m(nodAngle, nodCenterXY, hairPoints, minZ=60.0):
    nodHairPoints = []
    nodCenter = (nodCenterXY[1],minZ)
    for (x, y) in hairPoints:
        ptY = float(y)
        ptZ = minZ
        nodY, nodZ = rotateYZPlane(ptY, ptZ, nodCenter, nodAngle)
...
def saveTxtFile(txtPath, txtLines):
...
def turnHR(objLines, turnAngle, turnDirection, turnCenterMode):
    turnCenter = findTurnCenter(objLines, turnCenterMode)
    if turnDirection == 'left':
        turnedObjLines = turn3DModel(objLines, turnCenter, turnAngle)
    elif turnDirection == 'right':
