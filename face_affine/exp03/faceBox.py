...
def faceBoundingbox(faceImg):
    ...
    #               (bottomRightX, bottomRightY), (255, 0, 0), 2)
    topLeftX = int(topLeftX-width*0.3)
    topLeftY = int(topLeftY-height*0.6)
    bottomRightX = int(bottomRightX+width*0.3)
...
def trumpHeadpose(imgFolder, triTxtPath, headpose):
    ...
    videoWriter = cv2.VideoWriter(
        'faceBox_{}_{}.mp4'.format(headpose, time.strftime("%d%H%M", time.localtime())), cv2.VideoWriter_fourcc(*'mp4v'), 25, (1920, 1080))
